...
# mx,my,color分别为当前棋子的坐标位置和颜色
#计算某个方向的情况：
def caculate_num(mx,my,color,direction:tuple):
    global matrix
    list1 = []
    tx, ty = mx, my
    # 水平
    ...
    #direction为一个非负值的二元组，(1,0)代表计算水平方向的棋盘情况

# 返回值为该节点四个方向的棋盘情况
def get_list(mx, my, color):
    list_h = caculate_num(mx,my,color,(1,0))
    # 竖直
    list_v =caculate_num(mx,my,color,(0,1))
    # 左斜
    list_s =caculate_num(mx,my,color,(-1,1))
    # 右斜
    list_b =caculate_num(mx,my,color,(1,1))
    ...
    for i in range(min_tx1, max_tx1 + 1):
        for j in range(min_ty1, max_ty1 + 1):
            ...
            if matrix[i][j] == 0:
                ...
                # is_out()函数判断搜索范围是否超出边界，返回合法的搜索坐标范围(初始(1,1)到(19,19))
                min_tx2, min_ty2, max_tx2, max_ty2 = is_out(min_tx1, min_ty1, max_tx1, max_ty1)
                # 返回值为该节点四个方向的棋盘情况，i,j为当前搜索的棋子的坐标
                [list_h, list_v, list_s, list_b] = get_list(i, j, color_flag)
                # 评估分数 [list_h, list_v, list_s, list_b]为四个方向棋盘情况
                # 返回四个方向中最大的两个分值相加作为总分值
                eva1 = evaluate_node(list_h, list_v, list_s, list_b)
                Min = 100000
                # 代表是否需要接着遍历
                cut = False
# 根据当前棋面向前搜索2步，利用极大极小算法
def ai_go():
    global min_x, max_x, min_y, max_y, color_flag, matrix
    # 起始时间
    time_start = time.time()
    # evaluate_matrix 21*21 结点估值矩阵
    evaluate_matrix = [[0 for i in range(SIZE + 2)] for j in range(SIZE + 2)]
    if step == 0:
        if color_choice == 0: #用户选择白棋
            # add_chess(x, y, color) ai下第一步
            ...
            for i in range(min_tx1, max_tx1 + 1):
                for j in range(min_ty1, max_ty1 + 1):
                    ...
                    if matrix[i][j] == 0:
                        ...
                        for ii in range(min_tx2, max_tx2 + 1):
                            ...
                            for jj in range(min_ty2, max_ty2 + 1):
                                ...
                                if matrix[ii][jj] == 0:
                                    ...
                                    # 评估分数 [list_h, list_v, list_s, list_b]为四个方向棋盘情况
                                    # 返回四个方向中最大的两个分值相加作为总分值
                                    eva2 = evaluate_node(list_h, list_v, list_s, list_b)
                                    evaluate_matrix2[ii][jj]=eva2
                                    if eva2<Min:
                                        Min=eva2
                                    #恢复原来的棋局
                                    matrix[ii][jj]=0
                        ...
                        if evaluate_matrix[i][j]>Max:
                            Max =evaluate_matrix[i][j]
                            rx = i
                            ry = j
                        #恢复原来的棋局
                        matrix[i][j]=0
...
def add_chess(x, y, color):
    ...
    # 步数+1
    step = step + 1
    # 记录每个步骤的棋盘位置(x, y), 棋子颜色, 当前第几步
    # 如[(1, 1, -1, 1), (10, 10, 1, 2)]
    movements.append((x, y, color, step))
    # 棋盘矩阵标记当前棋盘每个棋子情况，1代表ai，-1代表玩家
    matrix[x][y] = color
    # 刷新棋盘已占有棋子的外切矩形范围 记录x,y的最大位置
    xy_range(x, y)
    # 判断游戏是否结束
...
# 玩家行棋
def player_go(pos):
    # x,y为当前棋子落点位置，如(1,1)代表第一行第一列(直线相交的位置)
    # x是列, y是行
    x = round(pos[0] / GRID_WIDTH)
    y = round(pos[1] / GRID_WIDTH)
    # 没有超出范围，并且该落点没有棋子，该棋子添加到棋盘
